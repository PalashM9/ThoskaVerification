...
def build_model(input_shape: tuple, weights: str, alpha: float,
    ...
    """ Constructing a constrained object detection model.
    ...
    #! First creating a full mobile_net_V2 from (HW, HW, C) input
    ...
    for layer in mobile_net_v2.layers:
        if type(layer) == BatchNormalization:
            layer.momentum = 0.9
    #! Cutting MobileNet where it hits 1/8th input resolution; i.e. (HW/8, HW/8, C)
    cut_point = mobile_net_v2.get_layer('block_6_expand_relu')
    #! Attaching a small additional head on the MobileNet
    #! 32 Filters
...
def train(num_classes: int, learning_rate: float, num_epochs: int,
    ...
    """ Constructing and train a constrained object detection model.
    ...
    #! Deriving output size from model
    ...
    #! Building weighted cross entropy loss specific to this model size
    weighted_xent = models.construct_weighted_xent_fn(model.output.shape, object_weight)

    #! Transforming bounding box labels into segmentation maps
    ...
    #! Initialising bias of final classifier based on training data prior.
    ...
    #! Creating callback that will do centroid scoring on end of epoch against
